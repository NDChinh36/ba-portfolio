...
sequenceDiagram
    ...
    TransferSvc->>UserSvc: GET /users/lookup?phone={recipient_phone} (resolved recipient wallet_id)
    Note over TransferSvc: user_id → wallet_id resolution:\nTransfer Service gọi UserService để lấy wallet_id\ntương ứng với sender_id và recipient_id trước khi\ngọi WalletCore.AtomicTransfer
...
<AccordionGroup>
  <Accordion title="[Sprint 1] Rate Limit theo Tier — Tier 3 cao hơn 5 GD/phút" icon="sliders">
  ...
  <Accordion title="[Sprint 1] Auto-Resolve GD PENDING — Timeout Policy" icon="clock-rotate-left">
  ...
  <Accordion title="[Sprint 2] Recent Recipients — Số lượng & TTL lưu trữ" icon="users">
  ...
  <Accordion title="[Sprint 2] Favorite Contacts — Đánh dấu thủ công hoặc tự động từ tần suất" icon="star">
  ...
  <Accordion title="[Sprint 2] SMS Fallback khi Recipient tắt Push Notification" icon="comment-sms">
    **Team:** Product · **Ưu tiên:** Medium
