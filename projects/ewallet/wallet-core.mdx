...
| BR-WALLET-14 | **LimitEngine bucket theo nhóm:** (1) TOPUP có bucket riêng; (2) WITHDRAWAL có bucket riêng; (3) **TRANSFER + QR_PAY dùng chung bucket** "DEBIT" — cộng gộp với nhau theo ngày/tháng (xem BR-QR-08). Ví dụ: Tier 2 limit ngày 100M; chuyển 70M + QR 40M = 110M → QR_PAY bị từ chối dù limit TRANSFER chưa đầy | Limit Engine, Transfer Service, QR Service |
...
| POST | `/internal/wallet/atomic-transfer` | `{sender_wallet_id, recipient_wallet_id, amount, idempotency_key, caller: "TRANSFER_SVC"}` *(internal — không phơi ra public API Gateway)* | `{tx_id, status: "COMPLETED"/"FAILED", sender_new_balance, timestamp}` | **Internal API** dành cho Transfer Service: thực hiện atomic debit/credit trong 1 DB transaction (BEGIN → debit → credit → COMMIT); gọi bởi Transfer Service sau khi limit check pass; row-lock theo thứ tự wallet_id nhỏ → lớn để tránh deadlock; kết quả ghi vào Transaction DB đồng thời |
...
| Giao dịch khởi tạo lúc 23:59:58 — limit check pass, nhưng DB commit lúc 00:00:02 (sang ngày mới) | Limit tính theo thời điểm **DB commit** (server timestamp); ngày reset 00:00 UTC+7; GD này tính vào ngày mới — developer phải dùng `now()` tại DB layer, không phải thời điểm request |
| Tài khoản ngân hàng đã liên kết bị ngân hàng đóng (account closed) kể từ lần topup cuối | Pull topup API trả lỗi từ ngân hàng → WALLET_007 / WALLET_008; App hiển thị "Ngân hàng không thể xử lý lúc này. Vui lòng kiểm tra tài khoản ngân hàng hoặc chọn ngân hàng khác."; không tự động unlink |

---

## 16. Cross-Module Data Flow

<Info>
  Phần này mô tả cách Wallet Core ghi dữ liệu sang các module khác sau mỗi giao dịch. Cần đọc bởi Developer và QA khi implement Transaction History và Notifications.
</Info>

### Wallet Core → Transaction History

Sau mỗi GD hoàn tất (COMPLETED hoặc FAILED), Wallet Core ghi bản ghi vào **transactions table** (chung với Transaction History module):

| Field | Kiểu | Mô tả | Nguồn |
|---|---|---|---|
| `tx_id` | UUID | Unique transaction ID | Wallet Core generate |
| `user_id` | UUID | Chủ ví (sender hoặc receiver) | Từ request |
| `type` | enum | `topup_pull`, `topup_push`, `withdrawal`, `p2p_send`, `p2p_receive`, `qr_pay` | Wallet Core classify |
| `direction` | enum | `credit` / `debit` | Wallet Core determine |
| `amount` | bigint (VND) | Số tiền tuyệt đối (không âm) | Từ request |
| `fee` | bigint | Phí GD (hiện tại = 0) | Fee engine |
| `status` | enum | `success`, `pending`, `failed` | Wallet Core outcome |
| `counterparty_name` | string | Tên đối tác (masked nếu cần) | User Service lookup |
| `description` | string | Nội dung chuyển khoản (nếu có) | Từ request |
| `napas_ref` | string? | Mã tham chiếu NAPAS (nullable — chỉ có với topup/rút/QR) | NAPAS response |
| `balance_after` | bigint | Số dư ví sau khi GD commit | Wallet Core snapshot tại commit |
| `created_at` | timestamp | Thời điểm GD khởi tạo | Server timestamp |
| `completed_at` | timestamp? | Thời điểm GD hoàn tất (nullable) | Wallet Core timestamp |

**Ghi chú cho các module khác:**
- **Transfer module**: Sau khi `AtomicTransfer` COMMIT, Wallet Core ghi 2 records: `p2p_send` cho sender + `p2p_receive` cho recipient
- **QR Payment module**: Ghi 1 record `qr_pay` cho buyer sau NAPAS credit confirm
- **Transaction History module**: Chỉ READ — không ghi trực tiếp vào transactions table
- **Cache invalidation**: Wallet Core emit event `WALLET_TX_CREATED` → Transaction Service bắt event → invalidate Redis cache `txlist_{user_id}_default`

### Wallet Core → KYC Module

Khi user KYC thành công và Tier thay đổi:
- KYC Service gọi `PATCH /internal/wallet/tier` với `{user_id, new_tier}` để Wallet Core cập nhật limit engine
- Wallet Core reload limit config cho user ngay lập tức (không cần restart)

---

## 17. Roadmap — Tính năng phát triển

<Info>
  Các tính năng và quyết định kỹ thuật dưới đây cần Tech Lead và Risk Team làm rõ trước Sprint tương ứng. Một số mục ảnh hưởng đến kiến trúc cốt lõi của Wallet Core và không thể dễ dàng thay đổi sau launch.
</Info>

<AccordionGroup>
  <Accordion title="[Sprint 1] Virtual Account Number — Fixed vs Rotating Strategy" icon="hashtag">
    **Team:** Tech Lead · **Ưu tiên:** Critical

    Quyết định chiến lược cấp **Số tài khoản ảo (STK ảo)** cho mỗi user khi nạp tiền từ ngân hàng: (A) **1 STK ảo cố định** per user — đơn giản, dễ dùng; hoặc (B) **STK ảo mới cho mỗi lần nạp** — an toàn hơn, giảm rủi ro track transaction pattern, nhưng phức tạp hơn về hạ tầng.

    **Yêu cầu chính:** Tech Lead chọn phương án và cập nhật API spec; document vào ADR; cập nhật SCR-WALLET-03 (màn nạp tiền).  
    **Phụ thuộc:** NAPAS / đối tác ngân hàng xác nhận khả năng cấp STK ảo dynamic.
  </Accordion>

  <Accordion title="[Sprint 1] Fraud Detection — Pattern Analysis NAPAS Timeout Retry" icon="shield">
    **Team:** Risk · **Ưu tiên:** High

    Bổ sung khả năng **phân tích pattern** cho chuỗi NAPAS timeout retry: phát hiện user cố tình trigger retry để khai thác race condition hoặc double-credit. Ghi log đầy đủ và alert Risk Team khi phát hiện pattern bất thường.

    **Yêu cầu chính:** Thêm `retry_count` và `retry_reason` vào transaction audit log; xây dựng alert rule (ví dụ ≥ 3 retry trong 10 phút / 1 user); Risk Team thiết lập threshold.  
    **Phụ thuộc:** Risk Team cung cấp fraud pattern ruleset; Tech Lead thiết kế log schema trước Sprint 1.
  </Accordion>

  <Accordion title="[Sprint 1] Policy Scope Lock khi NAPAS Đang Retry — Per-Wallet vs Global" icon="lock">
    **Team:** Tech Lead + Risk · **Ưu tiên:** High

    Xác định **phạm vi lock** khi đang xử lý NAPAS timeout retry: (A) **Lock toàn bộ ví** — an toàn nhưng block mọi giao dịch của user; hoặc (B) **Lock per-transaction** — cho phép GD độc lập chạy song song. Ảnh hưởng đến UX và tính nhất quán dữ liệu.

    **Yêu cầu chính:** Document policy vào BR-WALLET; cập nhật transaction flow; thêm trạng thái hiển thị phù hợp trên UI khi ví đang bị lock.  
    **Phụ thuộc:** Risk Team đánh giá nguy cơ double-spend của từng phương án; Tech Lead đánh giá complexity implement per-transaction lock.
  </Accordion>

  <Accordion title="[Sprint 2] Cảnh báo Hạn mức 80% — Channel & Frequency Policy" icon="bell">
    **Team:** Product · **Ưu tiên:** Medium

    Thiết kế chi tiết tính năng cảnh báo khi user đạt **80% hạn mức chi tiêu**: gửi qua **push notification** (cao priority) hay chỉ hiện **in-app banner** (ít intrusive)? Tần suất: cảnh báo mỗi GD sau 80% hay chỉ 1 lần duy nhất?

    **Yêu cầu chính:** Product quyết định channel và frequency; BA cập nhật Notification spec của module; implement trigger trong LimitEngine khi vượt ngưỡng 80%.  
    **Phụ thuộc:** User research về notification preference; cân nhắc notification fatigue nếu alert mỗi GD.
  </Accordion>

  <Accordion title="[Sprint 2] Real-time Balance Update — WebSocket / Server-Sent Events" icon="bolt">
    **Team:** Tech Lead · **Ưu tiên:** Medium

    Thêm cơ chế **real-time balance refresh** thay vì chỉ pull khi mở app. Khi nhận GD mới (chuyển tiền đến, nạp tiền thành công), số dư tự động cập nhật trên Home screen.

    **Yêu cầu chính:** Tech Lead lựa chọn WebSocket vs SSE; thiết kế reconnect strategy và backoff policy; cập nhật Home screen để subscribe balance events.  
    **Phụ thuộc:** Đánh giá impact lên battery / data consumption; infrastructure cần hỗ trợ persistent connection tại scale mục tiêu.
  </Accordion>
</AccordionGroup>
...
|---|---|---|
| Balance read API | P95 < 100ms | Redis cache; không đọc DB trực tiếp |
| Nạp tiền (pull) — commit response | P95 < 3s | Tổng thời gian từ PIN confirm đến credit ví |
| Nạp tiền (push) — webhook processing | P95 < 1s | Thời gian từ NAPAS webhook đến credit ví user |
| Rút tiền — NAPAS roundtrip | P95 < 5s | Phụ thuộc NAPAS SLA; P99 < 10s |
| Atomic debit/credit (DB transaction) | P95 < 200ms | PostgreSQL với row-level lock |
| Virtual account QR load | P95 < 300ms | Cached per user |

### Bảo mật (Security)

| Yêu cầu | Mô tả |
|---|---|
| Balance mutation authorization | Mọi thao tác ghi vào balance phải qua Wallet Core Service; không có direct DB write |
| DB constraint | `CHECK (balance_available >= 0)` và `CHECK (balance_total >= balance_available)` enforce tại DB level |
| Idempotency bắt buộc | Mọi topup/withdraw API phải kèm `idempotency_key`; server cache 24h |
| NAPAS webhook authentication | Verify chữ ký HMAC từ NAPAS; whitelist IP NAPAS; reject request thiếu chữ ký |
| Audit logging | Mọi balance mutation được log: user_id, amount, tx_type, before_balance, after_balance, ip, device_id, timestamp |

### Tính sẵn sàng & Độ tin cậy

| Chỉ số | Target | Ghi chú |
|---|---|---|
| Wallet Core Service uptime | ≥ 99.95% | Payment-critical; ≤ 4.4 giờ downtime/năm |
| NAPAS 247 dependency | ≥ 99.5% | NAPAS SLA cam kết; có retry 3 lần |
| Circuit breaker (NAPAS) | Mở sau 5 lỗi liên tiếp | Fallback: queue lệnh; notify user; tự đóng sau 60s |
| Graceful read-only mode | Nếu write path fail: vẫn hiển thị balance (read-only); không cho GD mới | Tránh total outage |
| DB replication | Sync replica cho reads; primary cho writes | Không single point of failure |

### Tính toàn vẹn dữ liệu & Ledger

| Yêu cầu | Mô tả |
|---|---|
| Double-entry ledger | Mỗi transaction tạo 2 ledger entries: debit (giảm source) + credit (tăng destination); tổng balance của hệ thống luôn = 0 |
| Daily reconciliation | Automated reconciliation mỗi 00:30 UTC+7: so sánh ledger entries vs. wallet balance vs. NAPAS statement |
| Alert on mismatch | Nếu reconciliation lệch > 0 đồng → alert ngay cho Finance + Tech Lead |

### Lưu trữ & Compliance

| Loại dữ liệu | Thời gian lưu | Cơ sở pháp lý |
|---|---|---|
| Transaction records (nạp, rút, phí) | Tối thiểu 5 năm | Nghị định 52/2024 |
| Ledger entries | Tối thiểu 5 năm | Accounting & audit requirement |
| Balance snapshots (daily) | 5 năm | Reconciliation & dispute resolution |
| NAPAS webhook raw payload | 1 năm | Technical audit; dispute với NAPAS |
