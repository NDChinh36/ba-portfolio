...
<Info>
  **Document ID:** PRD-EW-AUTH-001 · **Version:** 1.0 · **Status:** Draft  
...
| SCR-AUTH-15 | Cảnh báo thiết bị mới | Đăng nhập thành công trên device\_id lạ |
...
## 7. Sequence Diagram — Đăng nhập (PIN + Thiết bị mới)
...
sequenceDiagram
    ...
    User->>App: Nhập SĐT + PIN
    App->>AuthService: POST /auth/login {phone, pin_hash, device_id, device_info}

    AuthService->>AuthService: Lookup user by phone
    AuthService->>AuthService: Check account_status != LOCKED
    AuthService->>AuthService: Verify bcrypt(pin_hash)

    alt PIN đúng
        AuthService->>DeviceService: GET /devices?user_id&device_id
        DeviceService-->>AuthService: {known: false}

        AuthService->>TokenService: IssueTokens(user_id)
        TokenService-->>AuthService: {access_token, refresh_token}

        AuthService->>DeviceService: RegisterDevice(user_id, device_id, device_info)
        AuthService->>NotifService: PushAlert(user_id, "NEW_DEVICE_LOGIN", device_info)
        NotifService-->>AuthService: {sent: true}

        AuthService-->>App: 200 {access_token, refresh_token, new_device: true}
        App->>User: SCR-AUTH-15: Cảnh báo thiết bị mới

    else PIN sai (< 5 lần)
        AuthService->>AuthService: Increment failed_attempts
        AuthService-->>App: 401 {error: "INVALID_PIN",\nremaining_attempts: N}
        App->>User: "PIN không đúng. Còn N lần."
        ...
        AuthService-->>App: 423 {error: "ACCOUNT_LOCKED",\nunlock_at: timestamp}
        App->>User: "Tài khoản bị khóa. Thử lại sau X phút."
    end
```
...
### SCR-AUTH-15 — Cảnh báo thiết bị mới
...
│  │ 📱 iPhone 15 Pro          │  │
│  │ iOS 17.4 · Hà Nội         │  │
│  │ 26/05/2026 lúc 14:30      │  │
│  └───────────────────────────┘  │
│                                 │
│  Nếu đây là bạn, không cần làm  │
│  gì. Nếu không phải, đổi PIN    │
│  ngay để bảo vệ tài khoản.      │
│                                 │
│     [     Đổi PIN ngay     ]    │
│                                 │
│         Đây là tôi, tiếp tục    │
...
|---|---|---|---|
| Icon cảnh báo | Icon | Shield với dấu ! | — |
| Title | Text | "Đăng nhập từ thiết bị mới" | — |
| Device info | Card | Tên thiết bị · Hệ điều hành · Thời gian · Vị trí (IP geolocation) | — |
| Mô tả | Text | "Nếu đây là bạn, không cần làm gì. Nếu không phải, hãy đổi PIN ngay." | — |
| "Đổi PIN ngay" | Primary button | "Đổi PIN ngay" | Về SCR-AUTH-14 (Đổi PIN) |
| "Đây là tôi" | Secondary button | "Đây là tôi, tiếp tục" | Đóng popup; vào Home |
...
| BR-AUTH-09 | Đăng nhập thiết bị mới → gửi cảnh báo đến thiết bị cũ | SCR-AUTH-15, Notification |
...
| Thiết bị mới | Push (thiết bị cũ) + Email | Push: "Phát hiện đăng nhập mới từ {device} lúc {time}. Không phải bạn? Đổi PIN ngay." Email: Chi tiết device info, IP, thời gian, link đổi PIN | Khi phát hiện device mới |
...
| POST | `/auth/login` | `{phone, pin_hash, device_id, device_info}` | `{access_token, refresh_token, new_device?}` | Đăng nhập PIN |
...
| Hai thiết bị đăng nhập cùng lúc | Cả hai session đều hợp lệ; không force logout thiết bị cũ (chỉ cảnh báo) |
