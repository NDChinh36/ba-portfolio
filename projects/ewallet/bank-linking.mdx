...
|---|---|---|---|
| OQ-BANK-01 | Danh sách ngân hàng hỗ trợ Open Banking OAuth cụ thể gồm những ngân hàng nào? Mỗi bank có API riêng hay qua aggregator? | Tech Lead | Sprint 1 |
| OQ-BANK-02 | Fuzzy name match: nếu score 75–84%: automatic reject hay gửi manual review? | Compliance + Risk | Sprint 1 |
| OQ-BANK-03 | bank_access_token và refresh_token lưu ở đâu? HSM (Hardware Security Module) hay encrypted DB? | Security | Sprint 1 |
| OQ-BANK-04 | Nếu ngân hàng chưa hỗ trợ OAuth: có fallback về phương thức nhập STK + NAPAS lookup không? | Product | Sprint 2 |
| OQ-BANK-05 | Khi bank_refresh_token expire và user phải re-link: có cần notify trước không? Bao lâu trước? | Product + Tech | Sprint 2 |
