...
<Warning>
  **Thay đổi so với PRD-EW-AUTH-001:** Module này cập nhật hành vi đăng nhập thiết bị mới. Thay vì chỉ "cảnh báo sau khi login" (Auth v1.0), thiết bị mới **phải được thiết bị hiện tại xác nhận TRƯỚC** khi session được cấp. Cần cập nhật SCR-AUTH-15 trong Auth module.
</Warning>
...
sequenceDiagram
    ...
    alt Device A đang active
        ...
        DevB->>Auth: GET /auth/login/status {approval_token} (polling mỗi 3s)
...
- Body: "Bạn sẽ cần đăng nhập lại và được xác nhận từ thiết bị mới."
- CTA: [Hủy] | [Đăng xuất]
...
| GET | `/auth/login/status` | Device B poll trạng thái chờ approval | approval_token |
...
| Approval polling (Device B) | 3s interval | Không quá 100 polls/request |
...
|---|---|---|---|
| 1 | Nếu Device A hoàn toàn offline (mất điện, vứt máy) — user mới mua máy mới — flow CSKH xác minh và force-logout như thế nào? | Critical path cho trường hợp thực tế phổ biến; cần CSKH SOP | Sprint 8 — CSKH design |
| 2 | Giới hạn notification inbox: sau 90 ngày xóa hết — có cần archive/export không? | Compliance; user có thể cần biên lai thông báo | Sprint 10 |
| 3 | Push notification khi app bị kill hoàn toàn: approval notification có hiển thị action buttons trực tiếp trên notification shade không? | iOS: Notification Content Extension / Android: Reply action — cần native dev confirm | Sprint 7 tech |
| 4 | Nickname có hiển thị trong Transaction History (thay cho họ tên đầy đủ) không? | Nếu có: cần sync nickname vào transaction record hoặc join khi display | Sprint 8 |
| 5 | Email dự phòng có cần xác minh (verify email) không? | Nếu không verify → user có thể nhập email sai và không nhận được thông báo | Product decision Sprint 7 |
