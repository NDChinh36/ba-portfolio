...
<AccordionGroup>
  ...
  <Accordion title="[Sprint 8] Hiển thị Số dư sau GD trên danh sách" icon="wallet">
    ...
    **Yêu cầu chính:** DB thêm field `balance_after` vào bảng transactions tại thời điểm record GD; API `/transactions` trả thêm field; UX cập nhật row layout SCR-HIST-01.  
    **Phụ thuộc:** Data migration cho GD cũ không có `balance_after` (hiển thị "—"); đánh giá tăng payload size và ảnh hưởng bandwidth.
  </Accordion>
