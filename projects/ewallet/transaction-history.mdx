...
| Lọc theo trạng thái | ❌ | Không trong scope — danh sách default ẩn `failed`; user xem qua chi tiết |
...
|---|---|---|---|
| 1 | Giới hạn 6 tháng: nếu user muốn xem GD cũ hơn, có cần tính năng "Yêu cầu sao kê email" không? | CS ops + compliance; async export job | Sprint 9 |
| 2 | Push notification deep link: tap notification → mở thẳng SCR-HIST-04 với `tx_id`? | Deep link routing cần `tx_id` trong notification payload | Sprint 7 — confirm với Notif module |
| 3 | Search có cần filter kết hợp không? (vd: search "Nguyen" trong loại "Chuyển tiền") | UX improvement — search + filter combo | Sprint 9 |
| 4 | Hiển thị "Số dư sau GD" trên list row hay chỉ trên detail? | Nếu list → cần thêm field; tốn bandwidth | UX decision |
| 5 | Khi mở app sau khi có nhiều GD mới (từ push notification) — có badge số GD mới không? | UX — badge trên bottom tab "Lịch sử" | Sprint 8 |
